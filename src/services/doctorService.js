      ...
      if (!inputId) {
        ...
        let data = await db.User.findOne({
          ...
          include: [
            {
              model: db.Markdown,
              attributes: ["description", "contentHTML", "descriptionEn", "contentHTMLEn", 'contentMarkdown', 'contentMarkdownEn'],
            },
            {
              model: db.Doctor_Infor,
              attributes: {
      ...
      if (!doctorId) {
        ...
        let data = await db.User.findOne({
          ...
          include: [
            {
              model: db.Markdown,
              attributes: ["description", "contentHTML", "contentMarkdown", 'descriptionEn', 'contentHTMLEn', 'contentMarkdownEn'],
            },
            {
              model: db.Allcode,
              as: "positionData",
    ...
    try {
      if (
        ...
        <body> 
          ...
          <h3>Giá khám với bác sĩ: ${data.totalCostVi} VND - ${data.totalCostEn} $ </h3>
          <h3>Lưu ý: Giá này chưa bao gồm giá thuốc, bệnh nhân vui lòng xuống quày thuốc để trả tiền và tiếp nhận thuốc</h3>
        </body>
        </html>
        `;
        ...
        <body> 
          ...
          <h3>Consultation fee with the doctor: ${data.totalCostVi} VND - ${data.totalCostEn} $ </h3>
          <h3>Note: This price does not include the cost of medication. Patients are kindly requested to proceed to the pharmacy counter to make payment and receive their prescribed medications.</h3>
...
//history
let filterHistoryForUserService = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.patientId) {
        resolve({
        ...
        let dataHistories = []
        if (startDate && endDate) {
          dataHistories = await db.Booking.findAll({
            where: {
              patientId: data.patientId,
              statusId: ['S1', 'S2'],
              createdAt: {
            ...
            order: [['createdAt', 'DESC']],
            attributes: ["id", "patientId", "doctorId", "statusId", "date", "createdAt", "updatedAt"],
            include: [
              {
                model: db.Allcode,
                as: "statusData",
          ...
          dataHistories = await db.Booking.findAll({
            ...
            order: [['createdAt', 'DESC']],
            attributes: ["id", "patientId", "doctorId", "statusId", "patientReason", "createdAt", "date", "updatedAt",],
            include: [
              {
                model: db.Allcode,
                as: "statusData",
...
let deleteBookingSService = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.id) {
        resolve({
...
module.exports = {
  ...
  filterDoctors: filterDoctors, filterHistory, DeleteSchedule, filterHistoryForUserService, deleteBookingSService
};
