...
let sendSimpleEmail = async (dataSend) => {
  ...
  let info = await transporter.sendMail({
    from: '"dat lich kham benh 👻" <phu44650@gmail.com>', // sender address
...
let sendAttachment = async (dataSend) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // send mail with defined transport object
      let info = await transporter.sendMail({
        from: '" 👻" <phu44650@gmail.com>', // sender address
...
let sendConfirmationEmail = async (dataSend) => {
  // create reusable transporter object using the default SMTP transport
  let transporter = nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 587,
  ...
  let info = await transporter.sendMail({
    from: '" 👻" <phu44650@gmail.com>', // sender address
    to: dataSend.receiverEmail, // list of receivers
    subject: "Tạo tài khoản", // Subject line
    html: getBodyHTMLEmailCreateUser(dataSend),
  });
};

let getBodyHTMLEmailCreateUser = (dataSend) => {
  let result = "";
  result = `
<h3><b>Xin chào!</b></h3>
<p>Bạn nhận được email này vì đã yêu cầu tạo tài khoản trên website của chúng tôi</p>

<p>Nếu yêu cầu  là đúng sự thật, vui lòng click vào đường link bên dưới để hoàn tất thủ tục .</p>
<div><a href=${dataSend.redirectLink} target="_blank">Click here</a></div>

<div>Nếu bạn không yêu cầutạo tài khoản, hãy bỏ qua email này!</div>
<div>Xin chân thành cảm ơn!</div>
`;
  return result;
};
...
module.exports = {
  ...
  sendForgotPasswordEmail: sendForgotPasswordEmail, sendConfirmationEmail, getBodyHTMLEmailCreateUser
};
