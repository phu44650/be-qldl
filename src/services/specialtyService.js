  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (
        ...
        !data.descriptionMarkdown || !data.descriptionHTMLEn ||
        !data.descriptionMarkdownEn || !data.name
      ) {
        resolve({
          errCode: 1,
    ...
    try {
      if (!inputId || !location) {
        ...
        let data = await db.Specialty.findOne({
          where: { id: inputId },
          attributes: ["id", "name", "descriptionHTML", "descriptionMarkdown", "image", 'nameEn', 'descriptionHTMLEn', 'descriptionMarkdownEn'],
        });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (specialty) {
        ...
        specialty.descriptionHTMLEn = data.descriptionHTMLEn;
        specialty.descriptionMarkdownEn = data.descriptionMarkdownEn;
        //if (data.image) specialty.image = data.image;
        if (data.avatar) {
          specialty.image = new Buffer(data.avatar, "base64").toString("binary");
        }
...
module.exports = {
  ...
  udateSpecialtyData: udateSpecialtyData, filterSpecialtiesEn
};
